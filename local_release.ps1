...
if (Test-Path -LiteralPath $upx_dir -PathType Container) {
    pyinstaller.exe --onefile --windowed --upx-dir $upx_dir .\abst_gui.py
}else{
    # download UPX
    if (($upx.IsPresent)){
        ...
        # md -Force 
        pyinstaller.exe --onefile --windowed --upx-dir (($fname -split ".zip")[0]) .\abst_gui.py
    }else{
        pyinstaller.exe --onefile --windowed .\abst_gui.py
    }
